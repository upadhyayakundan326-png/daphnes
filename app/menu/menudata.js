const menuData = [
  {
    category: "Starters",
    items: [
      {
        name: "Paneer Tikka",
        image: "/tika.jpg",
        description: "Marinated cottage cheese grilled with Indian spices.",
        price: "₹249",
      },
      {
        name: "Crispy Chicken",
        image: "/chic.jpg",
        description: "Golden fried chicken served with spicy dip.",
        price: "₹299",
      },
      {
        name: "Veg Spring Roll",
        image: "/roll.jpg",
        description: "Crispy rolls stuffed with fresh vegetables.",
        price: "₹199",
      },
      {
        name: "French Fries",
        image: "/fries.jpg",
        description: "Crispy potato fries served with ketchup.",
        price: "₹149",
      },
    ],
  },

  {
    category: "Pizza",
    items: [
      {
        name: "Margherita",
        image: "/marg.jpg",
        description: "Classic cheese pizza with fresh basil.",
        price: "₹349",
      },
      {
        name: "Farmhouse",
        image: "/farmhouse.jpg",
        description: "Loaded with fresh vegetables and mozzarella.",
        price: "₹429",
      },
      {
        name: "Pepperoni",
        image: "/peporoni.jpg",
        description: "Premium pepperoni with extra cheese.",
        price: "₹499",
      },
      {
        name: "BBQ Chicken",
        image: "/bbq.jpg",
        description: "Smoky BBQ chicken topped with onions.",
        price: "₹529",
      },
    ],
  },

  {
    category: "Pasta",
    items: [
      {
        name: "White Sauce Pasta",
        image: "/white.jpg",
        description: "Creamy Alfredo sauce with herbs.",
        price: "₹289",
      },
      {
        name: "Red Sauce Pasta",
        image: "/red.jpg",
        description: "Classic tomato sauce with Italian seasoning.",
        price: "₹269",
      },
      {
        name: "Pink Sauce Pasta",
        image: "/pink.jpg",
        description: "Perfect blend of red and white sauce.",
        price: "₹299",
      },
    ],
  },

  {
    category: "Burgers",
    items: [
      {
        name: "Classic Veg Burger",
        image: "/veg.jpg",
        description: "Grilled veg patty with fresh lettuce.",
        price: "₹199",
      },
      {
        name: "Chicken Burger",
        image: "/chicken.jpg",
        description: "Juicy chicken patty with cheese.",
        price: "₹249",
      },
      {
        name: "Double Cheese Burger",
        image: "/dchees.jpg",
        description: "Loaded with double cheese and sauces.",
        price: "₹299",
      },
    ],
  },

  {
    category: "Coffee",
    items: [
      {
        name: "Espresso",
        image: "/espreso.jpg",
        description: "Strong and rich Italian espresso.",
        price: "₹149",
      },
      {
        name: "Cappuccino",
        image: "/capu.jpg",
        description: "Espresso topped with steamed milk foam.",
        price: "₹189",
      },
      {
        name: "Cafe Latte",
        image: "/late.jpg",
        description: "Smooth espresso with creamy milk.",
        price: "₹199",
      },
      {
        name: "Mocha",
        image: "/mocha.jpg",
        description: "Chocolate flavoured espresso delight.",
        price: "₹219",
      },
    ],
  },

  {
    category: "Desserts",
    items: [
      {
        name: "Chocolate Brownie",
        image: "/brownie.jpg",
        description: "Warm brownie served with chocolate sauce.",
        price: "₹179",
      },
      {
        name: "Cheesecake",
        image: "/cheesecake.jpg",
        description: "Creamy baked cheesecake with berry topping.",
        price: "₹249",
      },
      {
        name: "Tiramisu",
        image: "/tiramisu.jpg",
        description: "Classic Italian coffee flavoured dessert.",
        price: "₹279",
      },
    ],
  },

  {
    category: "Beverages",
    items: [
      {
        name: "Fresh Lime Soda",
        image: "/soda.jpg",
        description: "Refreshing sweet and salted lime soda.",
        price: "₹129",
      },
      {
        name: "Cold Coffee",
        image: "/coldcofee.jpg",
        description: "Chilled coffee blended with ice cream.",
        price: "₹199",
      },
      {
        name: "Fresh Orange Juice",
        image: "/orange.jpg",
        description: "100% freshly squeezed orange juice.",
        price: "₹179",
      },
    ],
  },
];

export default menuData;